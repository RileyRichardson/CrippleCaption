...
class Stream {
 public:
  ...
  void addAudio(const float* audioData, uint64_t audioLength,int32_t sampleRate = 16000);
...
class Transcriber {
 public:
  ...
  void addAudio(const std::vector<float> &audioData,int32_t sampleRate = 16000);
  void addAudio(const float* audioData,uint64_t audioLength,int32_t sampleRate = 16000);
...
inline void Stream::addAudio(const float* audioData, uint64_t audioLength,int32_t sampleRate){
    if(!audioLength)return;
    checkError(moonshine_transcribe_add_audio_to_stream(transcriber_->handle_, handle_,audioData,audioLength,sampleRate,0));
    streamTime_ +=static_cast<double>(audioLength) / static_cast<double>(sampleRate);
...
inline void Transcriber::addAudio(const float* audioData,uint64_t audioLength,int32_t sampleRate){
  getDefaultStream().addAudio(audioData,audioLength, sampleRate);
}
